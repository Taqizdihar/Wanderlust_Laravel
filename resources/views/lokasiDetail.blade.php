...
<body>
  ...
  {{-- Header atau navbar bisa include di sini --}}
  ...
  <div class="main">
    <div class="container">
      ...
      @if ($dataLokasi['status'] != 'active')
        <div class="actions">
          <a href="{{ url('propertiStatus?id=' . $dataLokasi['tempatwisata_id'] . '&status=active') }}" 
             class="edit-btn" id="active"
             onclick="return confirm('Are you sure you want to approve this property?')">
             Approve
          </a>
          <a href="{{ url('propertiStatus?id=' . $dataLokasi['tempatwisata_id'] . '&status=rejected') }}" 
